...
function run_const_sim()
    ...
    # ==============================================================================
    # 2. MAIN SIMULATION LOOP (Constant Speed Baseline)
    # ==============================================================================
    println("Starting Constant-Speed Baseline Simulation Loop...")
...
try
    println("Running constant-speed simulation...")
    clarity_history, speed_hist, lap_hist, lon_hist, lat_hist, salinity_meas_hist, weights_history = run_const_sim()

    # Save results to JLD2 file for later analysis
    filename = Dates.format(now(), "yyyy-mm-dd_HHMMSS") * "_const_speed_sim_results.jld2"
    ...
    savefig("const_speed_salinity_deviation_histogram.png")
    ...
    anim = @animate for k in frame_ids
        ...
        p1 = scatter(
            ...
            title = "Boat + Salinity\n$(Dates.format(sim_times[k], "yyyy-mm-dd HH:MM")) UTC | Lap $(lap_hist[k])",
    ...
    mp4(anim, "const_speed_results_nobattery.mp4"; fps = 15)
    ...
    p = plot(
        ...
        xlabel = "Time",
        ylabel = "Speed (m/s)",
        title = "Speed vs Time with Robot-Point Weight Overlay (Baseline)",
        label = "Speed",
        framestyle = :box,
        grid = true,
        legend = :topright,
        ylims = (0, maximum(speed_hist)*1.5) # Scaled up to keep the flat line visible below upper edge
    )

    plot!(
        twinx(), sim_times, weight_at_robot;
        lw = 2.2, lc = :crimson, ls = :dash,
        ylabel = "Hypothetical Target Weight",
        label = "Target Weight at Robot"
    )
    savefig("const_speed_weight_overlay.png")

    final_stats = "✅ Success! Total Clarity (Constant Speed): $(round(total_clarity, digits=2))"
